...
class TriPlaneGenerator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.triplane_superresolution = dnnlib.util.construct_class_by_name(class_name=rendering_kwargs['triplane_superresolution_module'], channels=32, img_resolution=rendering_kwargs['tri_res'], sr_num_fp16_res=0, sr_antialias=rendering_kwargs['sr_antialias'], **sr_kwargs)
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, patch_branch=False, patch_params=None, **synthesis_kwargs):
        ...
        if patch_branch:
            # highres planes for super resolution
            temp_planes = planes.view(len(planes)*3, 32, planes.shape[-2], planes.shape[-1])
    ...
    def sample(self, coordinates, directions, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ...
        planes = self.backbone.synthesis(ws, update_emas=update_emas, **synthesis_kwargs)
        planes = planes.view(len(planes), 3, 32, planes.shape[-2], planes.shape[-1])
        return self.renderer.run_model(planes, None, self.decoder, coordinates, directions, self.rendering_kwargs)
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, patch_branch=False, patch_params=None, **synthesis_kwargs):
        # Render a batch of generated images.
        ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        return self.synthesis(ws, c, update_emas=update_emas, neural_rendering_resolution=neural_rendering_resolution, cache_backbone=cache_backbone, use_cached_backbone=use_cached_backbone, patch_branch=patch_branch, patch_params=patch_params, **synthesis_kwargs)
